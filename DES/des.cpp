...
void Des::PermChoice1() //Permutation Choice-1 ген. Таблица 5.
{
int k=57,i;
for(i=0;i<28;i++) // Первые 28 бит
{
pc1[i]=key[k-1];
if(k-8>0)    k=k-8;
else	     k=k+57;
...
{
int exp[8][6],i,j,k;
//for( i=0;i<8;i++) // Таблица 2. Функция расширения E
//{
//    for( j=0;j<6;j++)
//    {
//        if((j!=0)||(j!=5))
...
void Des::PermChoice2() // Перестановка в соотв. с табл. 7
{
int per[56],i,k;
...
int i;
for(i=0;i<48;i++)
xor1[i]=expansion[i]^keyi[round-1][i]; // сложение по модулю 2 с ключом
}
...
void Des::substitution() // Преобразования S определяются таблицей 3.
{
int s1[4][16]={
...
{
    p=1;q=0;
    k=(a[i][0]*2)+(a[i][5]*1); // Первый и последний разряды являются двоичной записью числа а, (0<=a<=3)
    j=4;
    while(j>0) // средние 4 разряда представляют число b, (0<=b<=15).
    {
        q=q+(a[i][j]*p);
        p=p*2;
        j--;
    ...
    int d,i=3,a[4];
    while(v>0) // Записать v(найденную цифру) в двоичной системе
    {
        d=v%2;
        a[i--]=d;
        v=v/2;
...
void Des::inverse() // Конечная перестановка Таблица 8.
{
...
for(iB=0,nB=0,m=0;m<(strlen(Text)/8);m++) //Repeat for TextLenth/8 times.
{
    ...
    {
        Expansion(); //Performing expansion on `right[32]' to get  `expansion[48]' (1.функция расширения E)
        xor_oneE(round); //Performing XOR operation on expansion[48],z[48] to get xor1[48] (2.сложение по модулю 2 с ключом keyi[round])
        substitution();//Perform substitution on xor1[48] to get sub[32] (3.преобразование S, состоящее из 8 преобразований )
        permutation(); //Performing Permutation on sub[32] to get p[32] (4. перестановка P.)
    ...
    inverse(); //Inversing the bits of temp[64] to get inv[8][8] ( Конечная перестановка Таблица 8.)
...
*/
void Des::keygen() // Генерация 16 ключей
{
...
for(round=1;round<=16;round++)
{
    if(round==1||round==2||round==9||round==16) // определение смещения
        noshift=1;
    else
        noshift=2;
    while(noshift>0) // смещение 1 или 2 раза табл. 6
    {
        int t;
        t=ck[0]; //запомнить 1 бит
        for(i=0;i<28;i++) // смещение на -1
            ck[i]=ck[i+1];
        ck[27]=t;
        t=dk[0];
        for(i=0;i<28;i++)
    ...
    PermChoice2(); // Перестановка в соотв. с табл. 7
    for(i=0;i<48;i++)
        keyi[round-1][i]=z[i]; // Записать ключ в массив
}
}
